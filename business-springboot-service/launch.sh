...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "business-springboot-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name business-springboot-service apps/business-springboot-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app business-springboot-service:1.0-SNAPSHOT
    oc expose svc/business-springboot-service
else

	echo "Launching the application locally..."
	pattern="business-springboot-service"
	files=( $pattern )
	cd ${files[0]}
