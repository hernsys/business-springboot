...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "business-springboot-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name business-springboot-service apps/business-springboot-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app business-springboot-service:1.0-SNAPSHOT
		call oc expose svc/business-springboot-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd business-springboot-service
		cd target
